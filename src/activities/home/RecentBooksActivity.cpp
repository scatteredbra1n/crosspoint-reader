...
void RecentBooksActivity::render(RenderLock&&) {
  ...
  // Recent tab
  if (recentBooks.empty()) {
    renderer.drawText(UI_10_FONT_ID, safe.x + metrics.contentSidePadding, contentTop + 20, tr(STR_NO_RECENT_BOOKS));
  } else {
    GUI.drawList(
        renderer, Rect{safe.x, contentTop, safe.width, std::max(0, contentHeight)}, recentBooks.size(), selectorIndex,
